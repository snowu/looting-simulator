...
| Before | After | Sprite | What was wrong → what changed |
|---|---|---|---|
| 6 | 7 | Bog Seraph | Striking up close, a spatter of red dots at range. → First pass (a big grinning frog in a halo) was too on the nose. Now a frog seen from below, spread-eagled like an icon, inside a bone-gold halo, with feathers radiating through the ring, each with an eye near its tip. Attack: the halo ignites white, the eyes open red, the throat sac swells. |
...
| 4 | 7 | Your Shade (`shade_*`, `src/render/shade.ts`) | Borrowed the Hollow Knight under a pale wash, so it read as one more knight. → A faceless dark figure of its own: two cold eyes, smoking edges, legs gone by the knee, every pixel emissive so no light lifts it. It holds the weapon and shield you have equipped, composited into its hands at draw time in their materials, and those are the only clear things about it. The pale tint is gone. |
| 7 | 7 | Chest and mimic tell (`chest*`, `mimic_*`) | Owner's pick from the round-2 proposals. → A2 lid straps: two short dark iron straps down the lid (their ends show on the open lid's rim). The tell keeps the two nails and adds one pixel of tongue in the seam, and the mimic breathes: `chest_mimic_in` lifts the lid a pixel and darkens the seam for 1.1 s every 6.5 s, per-chest phase. |
| 7 | 8 | Awake mimic (`mimic_0`, `mimic_atk`) | The open chest with a small grin stamped over the gold, on scuffed stick legs. → All mouth: the lid is the upper jaw and the rim the lower, with uneven fangs (one gold), a dull red throat and a fat tongue over the rim. Centipede legs: seven small jointed legs a side along the bottom of the box, fanned out at the ends, every other one lifted on the attack so the frames scuttle (ten-row spider legs were too tall, and four crouched ones lost the owner's centipede idea). No eyes. The attack gapes wider, lengthens the fangs and lashes the tongue to the floor. Same 32×32 frame and scale as before. |
