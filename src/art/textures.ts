...
// ---------------------------------------------------------------------------
// Cracked walls (src/data/walls.ts)
// ---------------------------------------------------------------------------

/**
 * Three drawings laid over each biome's own wall, like the secret mark: a
 * plain fissure (a shortcut), a thin fissure with a few dull glints of ore (a
 * seam), and the faint, broken mortar line of a bricked-up niche with a
 * hairline crack (a cache). The last two are meant to be noticed on a second
 * look, not read from across the room.
 * Dark fissure `k` with a lit rim `l` on its right edge, so the crack reads as
 * a break in the stone rather than a line painted on it. The palettes are
 * translucent, so each wall's own colour shows through.
 */
const CRACK_ROWS = rows(`
  ................................
  ................................
  ..............k.................
  ..............kl................
...
const CRACK_PAL = { k: '#0a0806d8', l: '#e0d4b860' };
// Deliberately quiet: a seam is a thin fissure with a few dull glints, and a
// cache is a faint break in the mortar. Found by looking, not announced.
const SEAM_PAL = { k: '#0a0806b8', l: '#e0d4b83c', g: '#b8904890', G: '#e8d09ab0' };
const CACHE_PAL = { k: '#0a0806a0', m: '#0e0c0a58' };

/** Every wall a cracked tile can stand in. The throne floor has none. */
export const CRACKABLE_WALLS = ['wall_crypt', 'wall_mine', 'wall_cave', 'wall_catacombs', 'wall_burrows', 'wall_frostvault', 'wall_emberworks'];

export function crackTexture(wall: string, kind: 'shortcut' | 'seam' | 'cache'): string {
  return `${wall}_${kind === 'shortcut' ? 'cracked' : kind === 'seam' ? 'ore' : 'sealed'}`;
}

TEXTURES.push(...CRACKABLE_WALLS.flatMap((wall): ArtDef[] => [
  { id: crackTexture(wall, 'shortcut'), base: wall, palette: CRACK_PAL, rows: CRACK_ROWS },
  { id: crackTexture(wall, 'seam'), base: wall, palette: SEAM_PAL, rows: SEAM_ROWS },
  { id: crackTexture(wall, 'cache'), base: wall, palette: CACHE_PAL, rows: CACHE_ROWS },
]));
