...
const OVERRIDES = {
  ...
  ccc6336: 'Cashing in is easier and safer: one button on the market screen sells every valuable you own — the idols and chalices that were only ever coin — and anything a guild contract still needs wears a gold outline with a pennant, so you will know what you are giving up before you sell it.',
...
// Full bullet points for the two massive overhauls. The summary stays the
// lead; these are the specifics players actually argue about, with numbers.
const DETAILS = {
  bf6ef58: [
    'Blueprints are master recipes: feed spares into a recipe to climb Ranks 1→5 for +8%, +16%, +26%, +40% core stats and tougher gear. Unlocking costs 1 blueprint; each next rank costs its number.',
    'Secondary forge slots pull weight now — metal stiffens, wood quickens, hide heartens, cloth endures, bone brutalizes, all scaling with tier.',
    'Forged rarity follows the metal, up to 4 affixes on the finest pieces — a Master Smith squeezes out one more.',
    'Loot unlocks one band every two depths: Rare from 2, Epic from 4, Legendary at 6 only. High-tier metals and gems arrive later too.',
    'Blueprint drops chase recipes you have not learned and stop once one is maxed — no more dead drops.',
    'Vaults and secret rooms still pay, but the second guaranteed piece is rarer (50%→25%) and their gear rolls at your depth, not above it.',
  ],
  fb6bb03: [
    'Monsters grow with the floor number now, not just past their own front door — health climbs fastest, damage and armor trail behind. Deep floors are long fights, not coin flips.',
    'Armor cannot switch damage off: your blows always land at least a third through plate, and monster armor got wide enough to be a slope instead of a wall. Elements only half-ignore armor now.',
    'About a third fewer monsters per floor — fights last 3–7 swings instead of one, so the same crowd would be a queue. The Cave Spider’s lunge finally reads.',
    'Stamina returns at 22/s after 0.9s (was 34/s after half a second). Every swing is budgeted — Fight Milk’s numbers moved with it.',
    'Chests in 12% of ordinary rooms (was 35%), eight or nine lootable things a floor (was nineteen). Vaults and secret rooms pay full — the two places that are supposed to.',
    'Healing mends, never resets: Draughts 25%, Greater 55%, shrine fonts 60% plus your stamina back.',
    'The starting pack is 12 slots, not 16 — sixteen never filled, which had quietly deleted the whole “what do I carry home” game. Old saves lose nothing: overflow waits in the stash.',
    'Pack Mule 3/6/11, Supply Crate 5/11/18, Treasure Sense +20% a rank at 6/13/22 — priced on measured value. Find finally buys better loot, not more junk: it shrinks the Common pile and reaches chest valuables and gems. Fight Milk is found-only — the merchant’s shelf is dry.',
  ],
};
...
const SKIP = {
  ...
  e46dd4d: 'folded into the combined market note below',
